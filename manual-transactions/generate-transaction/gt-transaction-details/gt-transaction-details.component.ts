...
export class GtTransactionDetailsComponent {
  ...
  @Input() transactionQuantityExceedsAvailable: boolean = false;
  ...
  newToteIdFocusOut() {
    if (this.toteID != "") {
      this.iInductionManagerApi.ValidateTote({ toteID: this.toteID,orderNumber: this.orderNumber }).subscribe((res) => {
        if (res.isSuccess && !res.value.isValid) {
          this.toteID = "";
