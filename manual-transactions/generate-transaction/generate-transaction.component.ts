...
export class GenerateTransactionComponent implements OnInit {
  ...
  transactionQuantityExceedsAvailable: boolean = false;
  ...
  private handleTransactionQuantityExceedsAvailable(): void {
    this.transactionQuantityExceedsAvailable = true;
    this.global.ShowToastr(
      ToasterType.Error,
      showNotificationHeading.TransactionQuantityExceedsAvailable,
      ToasterTitle.Error
    );
    this.clearMatSelectList();
  }
  ...
  clearAfterPost(){
    ...
    this.transactionQuantityExceedsAvailable = false;
  ...
  postTransaction(type: string) {
    ...
    if (this.isLocation && this.transQuantity > actualQuantity && this.transType === TransactionType.Pick) {
      this.handleTransactionQuantityExceedsAvailable();
    } else {
      this.transactionQuantityExceedsAvailable = false;
      this.transactionQtyInvalid = false;
      this.clearMatSelectList();
  ...
  getLocationData() {
    ...
    this.iAdminApiService.LocationData(payload).subscribe((res: any) => {
      if (res?.isExecuted) {
        ...
        // Reset error state when location data changes as available quantity may have changed
        this.transactionQuantityExceedsAvailable = false;
      } else {
        this.global.ShowToastr(
          ToasterType.Error,
  ...
  onFieldValuesChangedOfTrans(fieldValues: any) {
    ...
    const previousTransQuantity = this.transQuantity;
    this.transQuantity = fieldValues.transQuantity;
    // Reset error state when transaction quantity changes
    if (previousTransQuantity !== this.transQuantity) {
      this.transactionQuantityExceedsAvailable = false;
    }
  }
