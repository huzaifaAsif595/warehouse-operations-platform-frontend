...
import { AssignedOrderLine } from 'src/app/bulk-process/bulk-transaction/interface/bulk-pick';
...
export class BulkTransactionComponent implements OnInit {
  ...
  async ProcessQuickPick() {
    ...
    const handleAfterLocationAssignment = async () => {
      ...
      if (locationAssigned) {
        ...
        // Populate orderLines from selectedOrders if empty
        if (!this.orderLines || this.orderLines.length === 0) {
          this.populateOrderLinesFromSelectedOrders();
        }

        // Create a Map for quick lookup by id
        const assignedMap = new Map<number, AssignedOrderLine>(
          assignedOrderLines.map(line => [line.id, line])
        );

        // Update locations and zones using the Map
        this.orderLines.forEach((line) => {
          const assignedLine = assignedMap.get(line.id);
          if (assignedLine) {
            this.updateOrderLineLocationAndZone(line, assignedLine);
          }
        });
          ...
          if (this.Prefernces?.workstationPreferences?.pickToTotes) {
            ...
            dialogRefTote.afterClosed().subscribe((result) => {
              if (result && result.selectedList && result.selectedList.length > 0) {
                ...
                // Repopulate orderLines from updated selectedOrders
                this.populateOrderLinesFromSelectedOrders();
  ...
  async getOrderLinesAssignedLocations(orderNumbers: string[]): Promise<AssignedOrderLine[]> {
    if (!orderNumbers || orderNumbers.length === 0) {
      return [];
    }
    try {
  ...
  private populateOrderLinesFromSelectedOrders(): void {
    this.orderLines = this.selectedOrders
      .flatMap(order => order.orderLines ?? []);
  }

  private updateOrderLineLocation(line: OrderLineResource, assignedLine: AssignedOrderLine): void {
    if (assignedLine.location != null) {
      line.location = assignedLine.location;
    }
  }

  private updateOrderLineZoneFromAssignedLine(line: OrderLineResource, assignedLine: AssignedOrderLine): boolean {
    if (assignedLine.zone != null && assignedLine.zone !== '') {
      line.zone = assignedLine.zone;
      return true;
    }
    return false;
  }

  private getZoneFromLocationZoneArray(location: string): string | null {
    const locationZoneData = this.locationZone.find(lz => 
      location && location.toLowerCase().startsWith(lz.zone.toLowerCase())
    );
    return locationZoneData ? locationZoneData.zone : null;
  }

  private extractZoneFromLocation(location: string): string | null {
    const locationStr = location || '';
    for (const zone of this.selectedZones) {
      if (locationStr.toLowerCase().startsWith(zone.toLowerCase())) {
        return zone;
      }
    }
    return null;
  ...
  private updateOrderLineZone(line: OrderLineResource, assignedLine: AssignedOrderLine): void {
    const zoneUpdated = this.updateOrderLineZoneFromAssignedLine(line, assignedLine);
    if (!zoneUpdated && line.location) {
      const zoneFromLocationZone = this.getZoneFromLocationZoneArray(line.location);
      if (zoneFromLocationZone) {
        line.zone = zoneFromLocationZone;
      } else {
        const extractedZone = this.extractZoneFromLocation(line.location);
        if (extractedZone) {
          line.zone = extractedZone;
        }
      }
    }
  }

  private updateOrderLineLocationAndZone(line: OrderLineResource, assignedLine: AssignedOrderLine): void {
    this.updateOrderLineLocation(line, assignedLine);
    this.updateOrderLineZone(line, assignedLine);
  }
